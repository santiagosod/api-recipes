...
const postIngredientsToUser = (req, res) => {
    const userId = req.user.id
    const { amount } = req.body
    const ingredientId = req.params.ingredient_id

    if (amount) {
        ingredientControllers.addIngredientsToUser({ userId, amount, ingredientId })
            .then(response => {
        ...
        res.status(400).json({ message: "Missing data", fields: { amount: "number" } })
    }
}
